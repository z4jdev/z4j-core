...
- **`z4j_core.celerybeat_compat` module**: framework-free translator
  for `CELERY_BEAT_SCHEDULE` entries. Maps celery `crontab`,
  `solar`, `timedelta`, and bare numeric / cron-string forms onto
  the z4j-native `ScheduleSpec` shape. Used by every adapter's
